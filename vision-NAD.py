...
def train_step(opt, train_loader, nets, optimizer, criterions, epoch):
    ...
    for idx, (img, target) in enumerate(train_loader, start=1):
        ...
        ##########################################################
        '''
        IRG + CE:84.04444444444445,1.8666666666666667
        ONLY IRG:85.13333333333334,2.1222222222222222 useful
        bad nets:[clean]Prec@1: 85.66 [bad]Prec@1: 12.37
        '''
        # if epoch == 10:
        ...
        ARGD:
        '''
        img_loss = criterionIMG([activation1_s, activation2_s, activation3_s, feat_s, output_s],
                                [activation1_t.detach(), activation2_t.detach(),
                                 activation3_t.detach(),
                                 feat_t.detach(),
                                 output_t.detach()])
        img_loss = cls_loss + img_loss
...
def train(opt):
    ...
    student = select_model(dataset=opt.data_name,
                           ...
                           pretrained=True,
                           pretrained_models_path="./weight/s_net/NAD.pth.tar",
                           n_classes=opt.num_class).to(opt.device)
    ...
    # AT_s = [activation1_s, activation2_s, activ
    #
    # ation3_s]
    ...
    criterions = {'criterionCls': criterionCls, 'criterionAT': criterionAT, 'criterionSP': criterionSP,
                  'criterionIRG': criterionIRG, 'criterionCC': criterionCC, 'criterionIMG': criterionIMG}
